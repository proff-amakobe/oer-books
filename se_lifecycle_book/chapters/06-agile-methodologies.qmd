...
![Agile Manifesto - Four Core Values](../assets/images/ch6_fig1_agile_manifesto.svg){width=72%}
...
![Scrum Framework](../assets/images/ch6_fig2_scrum_framework.svg){width=88%}
...
```         
   ...
   Boat Boat (Team)
     │
     │
  Anchor Anchor (What slows us down)
     │
  Wind Wind (What propels us forward)
     │
  Rocks Rocks (Risks ahead)
```
...
## Estimation: Story Points and Planning
...
├────────────────┬────────────────┬────────────────┬────────────────┬─────┤
│   Backlog Backlog   │   Ready Ready     │   In Progress In Progress│   Review Review   │ Done  │
│                │                │                 │               │Done │
├────────────────┼────────────────┼────────────────┼────────────────┼─────┤
...
## Breaking Down Work: Epics, Stories, and Tasks
...
[x] Story is written in user story format
[x] Acceptance criteria are defined
[x] Story has been estimated
[x] Dependencies are identified
[x] UX designs are complete (if applicable)
[x] Technical approach is understood
[x] Story is small enough for one sprint
[x] Product Owner is available to answer questions
```
...
[x] Code is deployed to staging
[x] QA has verified the feature
[x] Product Owner has accepted the work
[x] No known bugs remain
```
...
```         
Avoid: Each person reports to the Scrum Master
Avoid: No interaction between team members
Avoid: Runs long because people give detailed updates
Avoid: Feels like micromanagement
```

**The Endless Planning:**

```         
Avoid: Goes hours over time-box
Avoid: Debates implementation details
Avoid: No clear outcome
Avoid: Team disengages
```

**The Blameful Retrospective:**

```         
Avoid: Focus on who made mistakes
Avoid: Defensive atmosphere
Avoid: Same issues discussed repeatedly
Avoid: No action items
```

### Facilitating Effective Standups
...
FORMAT: Sailboat
         ...
         "Ship MVP by end of month"
            │
            │
   Boat Team (Current Position)
     │
     │
  Anchor Anchors (What's slowing us down?)
     • Unclear requirements on 2 stories
     • Staging environment unstable
     • Too many meetings
     │
  Wind Wind (What's pushing us forward?)
...
Example:
@alice: 
1. Done Completed Task CRUD API
2. 📝 Starting task assignment feature
3. 🚫 None

@bob:
1. Done Fixed date picker bug
...
## Adapting Agile to Your Context
