...
Pipeline executes automatically
    │
    ├── Success → Green checkmark Yes
    │
    └── Failure → Red X, team notified No
```
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  main branch:     Yes Build #1234 passed (3m ago)                        │
│  develop branch:  Yes Build #567 passed (15m ago)                        │
│  feature/auth:    No Build #89 failed - Test failure (1h ago)           │
...
```         
Avoid: "The build's been red for a week, but we're too busy to fix it."

Yes Fix broken builds immediately. A red build is an emergency.
```

**Infrequent Integration:**

```         
Avoid: Committing once a week with massive changes

Yes Commit multiple times daily with small changes
```

**Skipping Tests:**

```         
Avoid: "I'll add tests later" or "Tests are too slow, skip them"

Yes Tests are non-negotiable. Optimize slow tests.
```

**Not Running Pipeline Locally:**

```         
Avoid: "It works on my machine" → Push → CI fails

Yes Run the same checks locally before pushing
```

**Long-Lived Feature Branches:**

```         
Avoid: Feature branch that diverges for months

Yes Short-lived branches, merged within days
```
...
[x] Custom deployment branch policy
...
// Yes Safe: Adding a column with default
...
``` javascript
// No Dangerous: Renaming column (breaks running code)
    ...
    steps:
      ...
      - name: Notify team
        ...
        with:
          payload: |
            {
              "text": "In Progress Rollback completed for ${{ github.event.inputs.environment }}",
              "blocks": [
                {
                  "type": "section",
                  "text": {
