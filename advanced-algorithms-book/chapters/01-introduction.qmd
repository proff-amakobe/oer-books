...
```
O(1) < O(log log n) < O(log n) < O(n^(1/3)) < O(√n) < O(n) < O(n log n) < O(n²) < O(n³) < O(2ⁿ) < O(n!) < O(nⁿ)
```
...
def demonstrate_best_vs_worst_case():
    ...
    print(f"\n💡 Insight: Insertion sort is {worst_time/best_time:.0f}x slower on reverse-sorted data!")
