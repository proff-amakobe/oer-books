...
# Introduction & Algorithmic Thinking

_"The best algorithms are like magic tricks—they seem impossible until you understand how they work."_

---

## Welcome to the World of Advanced Algorithms {.unnumbered}
...
### Why Study Advanced Algorithms? {.unnumbered}
...
Input: A list of scores S = [s₁, s₂, ..., sₙ]
...
```{.javascript .program-code chapter=1}
...
_Base case:_ After processing 1 element (k=1), max_score = s₁, which is trivially the maximum of {s₁}.
...
_Conclusion:_ After processing all n elements, max_score contains the maximum value in the entire list. ✓
...
## The Eternal Trade-off: Correctness vs. Efficiency
...
## Asymptotic Analysis—Understanding Growth
...
|Input Size (n)|Algorithm A (100n μs)|Algorithm B (n² μs)|Which is Faster?|
|---|---|---|---|
|10|1,000 μs|100 μs|B is 10x faster|
|100|10,000 μs|10,000 μs|Tie!|
|1,000|100,000 μs|1,000,000 μs|A is 10x faster|
|10,000|1,000,000 μs|100,000,000 μs|A is 100x faster|
...
Here, $f(n)$ is the quantity being bounded, $g(n)$ is the comparison function, $c$ is a fixed scaling constant, and $n_0$ is the threshold beyond which the bound holds. Both functions are assumed nonnegative for sufficiently large $n$.

**In plain English:** $f(n)$ grows no faster than $g(n)$, up to a constant factor and for sufficiently large $n$. This is an asymptotic upper bound, not necessarily an exact growth rate.

**Visual Intuition:** Imagine drawing $f(n)$ and $c\,g(n)$ on a graph. After $n_0$, the curve $c\,g(n)$ stays above $f(n)$.
...
For large $n$, the terms $5n$ and $2$ become negligible compared with $3n^2$. More precisely, for $n\ge1$:

- $5n\le5n^2$, since $n\le n^2$.
- $2\le2n^2$, since $1\le n^2$.
...
Thus $c=10$ and $n_0=1$ prove that $3n^2+5n+2=O(n^2)$.
...
0 \le c\,g(n) \le f(n)
\qquad \text{for all } n \ge n_0.
$$ {#eq-bigomega-definition}

The variables have the same meanings and eventual-nonnegativity assumption as in @eq-bigo-definition. **In plain English:** $f(n)$ grows at least as fast as $g(n)$, up to a constant factor.
...
Thus we may choose $c=3$ and $n_0=1$.

#### Big-Θ Notation (Tight Bound)

**Definition:** We say $f(n)=\Theta(g(n))$ when $f(n)=O(g(n))$ and $f(n)=\Omega(g(n))$.

**In plain English:** $f(n)$ and $g(n)$ have the same asymptotic growth rate up to constant factors. This does not mean that the functions are numerically equal.

**Example:** Since we proved both an upper and a lower bound,

$$
3n^2+5n+2=\Theta(n^2).
$$

For sufficiently large $n$, this polynomial therefore grows quadratically.
...
❌ **Wrong thinking:** "Algorithm A takes 1000n² time, Algorithm B takes n² time. Since both are $O(n^2)$, they're equally good."
...
$$
\begin{aligned}
O(1)&\subset O(\log\log n)\subset O(\log n)\subset O(n^{1/3})\subset O(\sqrt n),\\
&\subset O(n)\subset O(n\log n)\subset O(n^2)\subset O(n^3),\\
&\subset O(2^n)\subset O(n!)\subset O(n^n).
\end{aligned}
$$

The inclusions compare asymptotic upper-bound classes; they describe eventual growth and do not rank running times for every finite input.
...
- 25! = 1.5 × 10²⁵ (more than the number of atoms in the observable universe!)
...
- **Time Complexity:** $O(n^2)$ (the n² term dominates)
...
- If we start with n elements: n → n/2 → n/4 → n/8 → ... → 1
...
## Setting Up Your Algorithm Laboratory
...
## Description
My implementation and evaluation of advanced algorithms.

## Setup
\`\`\`bash
python -m venv algorithm_lab
source algorithm_lab/bin/activate  # On Windows: algorithm_lab\Scripts\activate
...
## Current Progress
- [x] Milestone 1: Environment setup and basic analysis
- [ ] Milestone 2: Sorting algorithms
- [ ] Milestone 3: Search algorithms

## Author
Add the project author's name and preferred contact channel.
EOF
...
def main():
    ...
    try:
        ...
        if correctness_passed:
            ...
            print("\n⚠️  Skipping performance demos due to correctness failures.")
...
**Part A:** Prove using formal definitions that 5n³ + 3n² + 2n + 1 = $O(n^3)$

- Find appropriate constants c and n₀
- Show your work step by step
- Justify each inequality

**Part B:** Prove using formal definitions that 5n³ + 3n² + 2n + 1 = $\Omega(n^3)$
...
**Part D:** Prove or disprove: 2n² + 100n = $O(n^2)$
...
2. **Optimized Trial Division:** Test divisibility by numbers from 2 to √n, skipping even numbers after 2
...
1. Generate datasets of various sizes (10² to 10⁵ elements)
...
Instructors adopting these exercises should publish their own schedule, submission process, collaboration expectations, and support channels alongside the course syllabus. These administrative details are intentionally not prescribed by the textbook.
