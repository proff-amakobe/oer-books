...
**From asymptotic analysis to dependable evidence**

## From Algorithm Analysis to Algorithm Engineering

An asymptotic bound describes how resource use grows; an engineered implementation shows how that algorithm behaves under explicit assumptions on a real system. Algorithm engineering connects these views through careful implementation, testing, measurement, and interpretation.
...
- define interfaces and invariants between algorithmic components;
- construct correctness, integration, and regression tests;
- design repeatable benchmarks with controlled inputs and environments;
- distinguish asymptotic improvement from constant-factor and systems effects;
- report uncertainty, limitations, and reproducibility information; and
- communicate performance conclusions without exceeding the evidence.

This chapter develops a repeatable workflow for turning an algorithmic prototype into a dependable experimental implementation.
...
The same workflow applies to research prototypes, production experiments, and independent projects.

## Building Reliable Experimental Implementations
...
```{.python .program-code chapter=14}
...
An asymptotically efficient algorithm can still perform poorly on relevant inputs, and a fast result on one machine can be an artifact of caching, compilation, or measurement noise. Benchmarking connects the cost model to observed behavior.
...
Concentrated bottlenecks are common, but their location and magnitude must be measured rather than assumed.

### Experimental Protocol

Before collecting results, specify the research question, competing implementations, input distributions, correctness oracle, measured quantities, and stopping rule. Include adversarial or boundary cases when the algorithm's analysis predicts different behavior from average inputs.

Run warm-up iterations when the language runtime, cache hierarchy, or accelerator has startup effects. Then collect multiple independent measurements and report the distribution—such as median and interquartile range or a confidence interval—rather than only the fastest run. Retain raw observations so that plots and summary statistics can be regenerated.

Record the processor, memory, operating system, runtime and compiler versions, dependency lockfile, parallelism settings, random seeds, and relevant power or accelerator configuration. Compare implementations under equivalent conditions and verify outputs before comparing speed. A log-log slope can support a growth-rate hypothesis, but finite measurements do not prove an asymptotic bound.
...
**Impact**: Can change complexity class (e.g., $O(V^2)$ → $O(V+E)$ for sparse graphs).

#### **For Dynamic Programming**

**Optimization**: Use space-optimized versions when you only need the last row/column.

**Impact**: $O(n^2)$ space → $O(n)$ space, enables solving larger instances.
...
````{.markdown .configuration chapter=14}
# Advanced Data Structures Library

Efficient advanced data structures for competitive programming
...
Contributions are welcome; include contribution instructions with the project repository.
...
**Design**:
- ☐ Is the design clear and logical?
...
**Example opening**:
> "Imagine you're analyzing DNA sequences—3 billion letters long. You need to find patterns, but traditional string matching would take hours. What if I told you we could do it in seconds? That's what suffix arrays enable, and today I'll show you how."
...
**✅ Artifact packaging**:
- Source, tests, and scripts are versioned
- Access conditions and license are stated
- README links the report and generated results
- Environment and dependency versions are recorded

## Summary and Exercises

This chapter connected algorithm analysis to evidence-producing implementation:

- **System integration**: Making components work together seamlessly
- **Benchmark design**: Measuring repeated trials under controlled conditions
- **Reproducibility**: Recording inputs, environments, randomness, and raw results
- **Testing and review**: Ensuring quality and correctness
- **Presentation**: Communicating your work effectively

**Exercises**:

1. Design a benchmark that distinguishes two implementations with the same asymptotic complexity. State the input distribution and uncertainty summary.
2. Reproduce one timing result on a second environment and explain plausible causes of any difference.
3. Identify three threats to validity in a published algorithm benchmark and propose controls for each.
