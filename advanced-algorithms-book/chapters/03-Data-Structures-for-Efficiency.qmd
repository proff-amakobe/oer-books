...
## When Algorithms Meet Architecture {.unnumbered}
...
## Introduction: The Hidden Power Behind Fast Algorithms {.unnumbered}
...
### Why Data Structures Matter {.unnumbered}
...
### What Makes a Good Data Structure? {.unnumbered}
...
![A max-heap with root 50 and children 30 and 40 maps level by level to array indices zero through six.](../assets/figures/chapter03/binary-heap-array-mapping.svg){#fig-heap-array-mapping width=78% fig-pos="H" fig-cap="A complete binary heap maps directly to a contiguous array by level order."}

For array index $i$, the parent is $(i-1)\mathbin{//}2$, the left child is $2i+1$, and the right child is $2i+2$.

### Core Heap Operations

```{.python .program-code chapter=3}
class MaxHeap:
    """
...
## Hash Tables - $O(1)$ Average Case Magic
...
Fibonacci heaps obtain their strongest bounds through amortized analysis, including constant amortized time for decrease-key [@fredman1987fibonacci].
...
## Project - Comprehensive Data Structure Library
