# Advanced Graph Algorithms - Making Things Flow
...
## Introduction: The Universal Language of Flow
...
![A source branches to vertices a and b with capacities 10 and 5; their outgoing edges to the sink have capacities 10 and 15.](../assets/figures/chapter09/simple-flow-network.svg){#fig-simple-flow-network width=68% fig-pos="H" fig-cap="Two edge-disjoint paths carry a maximum flow of 15 units."}
...
### The Genius Idea: The Residual Network
...
![For an edge carrying seven of ten units, the residual network contains forward capacity three and a dashed backward edge of capacity seven.](../assets/figures/chapter09/residual-edge.svg){#fig-residual-edge width=64% fig-pos="H" fig-cap="Residual edges represent both unused forward capacity and flow that may be canceled."}
...
## The Max-Flow Min-Cut Theorem: One of CS's Greatest Hits
...
![A dashed boundary immediately after the source crosses capacity-10 and capacity-5 edges while highlighted paths carry total flow 15.](../assets/figures/chapter09/max-flow-min-cut.svg){#fig-max-flow-min-cut width=74% fig-pos="H" fig-cap="The source cut has capacity 15, equal to the displayed maximum flow."}
...
## Ford-Fulkerson: The Classic Algorithm
...
### Edmonds-Karp: Breadth-First Augmenting Paths
...
**Edmonds-Karp** obtains a polynomial running-time bound by choosing shortest augmenting paths in the residual graph [@edmonds1972networkflow]:
...
### Why BFS Makes All the Difference
...
## Push-Relabel: The Modern Approach
...
- ❌ $O(VE^2)$ can be slow on dense graphs

**Push-Relabel**:
- ✅ Faster on dense graphs ($O(V^3)$ or better with optimizations)
...
## Bipartite Matching: When Flow Solves Romance
...
![A source connects to three left vertices, compatibility edges connect the bipartition, and three highlighted unit-flow paths reach the sink through distinct right vertices.](../assets/figures/chapter09/bipartite-matching-flow.svg){#fig-bipartite-matching-flow width=82% fig-pos="H" fig-cap="Unit capacities convert bipartite matching into maximum flow while enforcing at most one match per vertex."}
...
## Minimum Cost Flow: Optimizing While Flowing
...
## Chapter Project: Universal Flow Network Solver
...
2. **Edmonds-Karp**: Use BFS for paths → $O(VE^2)$ time
3. **Push-Relabel**: Local "push excess downhill" approach → $O(V^3)$ time
