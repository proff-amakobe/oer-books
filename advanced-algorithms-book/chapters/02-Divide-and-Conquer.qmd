...
![A balanced binary recursion tree has cn total work at every level, from one root problem through n constant-size leaves.](../assets/figures/chapter02/merge-sort-work-tree.svg){#fig-merge-sort-work width=84% fig-pos="H" fig-cap="Merge sort performs Θ(n) work per level across Θ(log n) levels."}
...
![Balanced quicksort partitions form a logarithmic-height tree; repeatedly choosing an extreme pivot forms a linear chain.](../assets/figures/chapter02/quicksort-partition-comparison.svg){#fig-quicksort-partitions width=88% fig-pos="H" fig-cap="Partition balance determines whether quicksort has Θ(n log n) or Θ(n²) recursion."}
...
The unbalanced case is the right-hand tree in @fig-quicksort-partitions: its level costs sum to an arithmetic series, giving Θ(n²) work.
...
**Case 2:** If f(n) = Θ(n^(log_b(a))), then:

```{.inline-example chapter=2}
T(n) = Θ(n^(log_b(a)) log n)
```

**Case 3:** If f(n) = Ω(n^(log_b(a) + ε)) for some constant ε > 0, AND if af(n/b) ≤ cf(n) for some constant c < 1 and sufficiently large n, then:
...
|f(n) = Ω(n^(E+ε)), ε > 0<br>AND regularity holds|Case 3|T(n) = Θ(f(n))|
