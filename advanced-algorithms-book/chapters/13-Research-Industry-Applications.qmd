...
```{.python .program-code chapter=13}
import random
...
- Edit distance requires $\Omega(n^2)$
- Longest common subsequence requires $\Omega(n^2)$
...
Grover's algorithm gives a quadratic quantum query improvement for unstructured search, from linear to $O(\sqrt N)$ oracle queries [@grover1996search].
...
- Sorting: Still $\Omega(n log n)$ (maybe √n speedup on some measures)
...
$$
\mathcal M(D)=f(D)+Z,
\qquad Z\sim\operatorname{Laplace}\!\left(0,\frac{\Delta f}{\epsilon}\right),
$$
...
$$
\Pr[\mathcal M(D)=o]\propto
\exp\!\left(\frac{\epsilon\,q(D,o)}{2\Delta q}\right),
$$
...
\approx
\frac{L(\theta+\epsilon e_i)-L(\theta)}{\epsilon}.
$$
...
$$
\begin{aligned}
\frac{\partial L}{\partial\widehat y}&=2(\widehat y-y),\\
\frac{\partial L}{\partial W_n}
  &=\frac{\partial L}{\partial\widehat y}
    \frac{\partial\widehat y}{\partial W_n},\\
\frac{\partial L}{\partial h_{n-1}}
  &=\frac{\partial L}{\partial\widehat y}
    \frac{\partial\widehat y}{\partial h_{n-1}}.
\end{aligned}
$$
...
For a sampled example $(x,y)$, SGD computes $\nabla_\theta L(f(x;\theta),y)$ and applies
...
Here $g_t=\nabla_\theta L_t$; squares, roots, and division are applied componentwise.
...
**Time complexity**: $O(n^2d)$ where n = sequence length, d = dimension
...
\qquad
\operatorname{output}=\operatorname{Concat}(\operatorname{head}_1,\ldots,\operatorname{head}_h)W_O.
$$
...
The actor is $\pi(a\mid s;\theta)$ and the critic is $V(s;w)$; the critic's value estimate serves as a baseline for the actor update.
...
```{.markdown .configuration chapter=13}
