...
| **The Loss Landscape**                                                                                                                                                                                                                |
|                                                                                                                                                                                                                                       |
| *A 3D surface showing peaks, valleys, saddle points, and plateaus. Gradient descent trajectories from multiple starting points converge toward low-loss regions at different rates, depending on learning rate and optimizer choice.* |
+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|                                                                                                                                                                                                                                               |
| *Three training loss curves: too high (erratic, possibly diverging), just right (smooth descent to low loss), too low (slow descent that plateaus far from minimum). A fourth curve shows learning rate warmup followed by cosine annealing.* |
+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|                                                                                                                                                                                                             |
| *Three descent paths through a contoured loss landscape. Batch: clean, efficient path. SGD: noisy, zigzag path. Mini-batch: moderately noisy but generally descending. Speed-stability tradeoff annotated.* |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|                                                                                                                                                                                                                                            |
| *A circular flow diagram: Forward Pass → Loss Computation → Backward Pass (Backpropagation) → Parameter Update → repeat. Each phase color-coded and labeled with plain-language descriptions and the key mathematical operation involved.* |
+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|                                                                                                                                                                                                                                                                        |
| *A single graph overlaying four training curves: constant rate, step decay, cosine annealing, and warmup followed by cosine annealing. Final validation accuracy labeled for each. The warmup + cosine curve achieves the best result with the smoothest convergence.* |
+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+

**Figure 3.9:** Schedule Comparison. *Schedule choice can matter as much as architecture choice. The same model, trained identically except for its learning rate schedule, can produce substantially different outcomes.*
...
Rather than sharp steps, cosine annealing decreases the learning rate smoothly following a cosine curve --- from its maximum value to near zero across the training run. The smooth deceleration avoids the instability of step decay and typically produces better final performance. Cosine annealing can also be combined with warm restarts: periodically resetting the learning rate back to its maximum value, allowing the optimizer to escape shallow local minima and explore different regions of the loss landscape before decelerating again.
...
|                                                                                                                                                                                                                                                                                                      |
| *Four learning curve panels: underfitting (both losses high), good fit (both losses low and tracking), mild overfitting (training low, validation plateau), severe overfitting (training low, validation rising). Each panel labeled with a diagnostic interpretation and a suggested intervention.* |
+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
| **How Dropout Works**                                                                                                                                                                                                                                                                        |
|                                                                                                                                                                                                                                                                                              |
| *Left: full network active. Middle: dropout applied during training --- grayed neurons are inactive in this step. Right: different neurons inactive in the next step. Far right: all active at test time, outputs scaled. The pattern of \'coverage\' across training steps is illustrated.* |
+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
|                                                                                                                                                                                                                                                                                        |
| *A deep network shown as a chain of layers. Vanishing case: gradient arrows shrink to near-invisible toward early layers. Exploding case: arrows grow to catastrophic size. Right panel: training curves for each pathology --- flat/zero for vanishing, exploding/NaN for exploding.* |
+----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
| **\[ FIGURE 3.10 \]**                                                                                                                                                                                                                                                                                                                     |
|                                                                                                                                                                                                                                                                                                                                           |
| **MIPDS Training Engine --- Week 3 State**                                                                                                                                                                                                                                                                                                |
|                                                                                                                                                                                                                                                                                                                                           |
| *System diagram of the MIPDS vision component (from Week 2) with training infrastructure overlaid: loss function, optimizer, regularization layers (Dropout), data augmentation pipeline, validation loop, learning rate schedule, and checkpoint manager. Labels connect each component to its conceptual motivation from this chapter.* |
+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
